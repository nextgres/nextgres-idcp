...
static const PQcommMethods NGPqCommSocketMethods = {
  socket_comm_reset,
  socket_flush,
 ...
 */
void
ng_idcp_pq_init(void)
{
  int      socket_pos PG_USED_FOR_ASSERTS_ONLY;
  int      latch_pos PG_USED_FOR_ASSERTS_ONLY;

  PqCommMethods = &NGPqCommSocketMethods;
...
int
ng_idcp_stream_server_port(int family, const char *hostName, unsigned short portNumber,
         ...
         pgsocket ListenSocket[], int MaxListen)
{
  ...
  for (addr = addrs; addr; addr = addr->ai_next)
  {
    ...
    if (addr->ai_family != AF_UNIX)
    {
      ...
      if ((setsockopt(fd, SOL_SOCKET, SO_REUSEPORT, (char *)&one,
                      sizeof(one))) == -1) {
        ereport(LOG,
                (errcode_for_socket_access(),
                 errmsg("%s(%s) failed for %s address \"%s\": %m",
                        "setsockopt", "SO_REUSEPORT", familyDesc, addrDesc)));
        closesocket(fd);
        continue;
      }
    }
 ...
 */
int
ng_idcp_stream_connection (pgsocket server_fd, Port *port)
{
 ...
 */
int
ng_idcp_pq_getbytes(char *s, size_t len)
{
  size_t    amount;
 ...
 */
bool
ng_idcp_pq_buffer_has_data(void)
{
 ...
 */
bool
ng_idcp_pq_is_reading_msg(void)
{
 ...
 */
int
ng_idcp_pq_getmessage(StringInfo s, int maxlen)
{
  int32    len;
